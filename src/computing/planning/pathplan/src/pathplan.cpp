...
class PathPlan {
			...
			object_pub(nh.advertise<pathplan::kusv_ObjectCmd>("/object_cmd", 500))                             //js
	{
		...
		m_pListener->waitForTransform (BASE_FRAME, LOCAL_FRAME, ros::Time(0), ros::Duration (10));
		...
		void calcDistan(const geometry_msgs::Point point){
			...
			if(a1==0){
				...
				double order1 = 9*a*a*pow(point.x, 4.0);
				double order2 = 12*a*b*pow(point.x, 3.0);
				double order3 = 4*b*b*pow(point.x, 2.0);
				double order4 = 4*b*c*point.x;
				double in = order1 + order2 + order3 + order4 + c*c + 1;
				distan = sqrt(in);
