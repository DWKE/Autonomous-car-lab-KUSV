...
void TaskNode::Init(){
    ...
    // Register Publisher
    ros::NodeHandle nh;
    m_pub_estimated_gnss = nh.advertise<sensor_msgs::NavSatFix>("estimated_gnss", 10);
    m_pub_estimated_heading = nh.advertise<sensor_msgs::Imu>("estimated_heading", 10);
    m_pub_gps_status = nh.advertise<std_msgs::Float32>("gps_status",10);
...
void TaskNode::PublishEstimatedGnss(){
    ...
    //ROS_INFO("DiffTIme : %f",dDiffTime_sec);
...
void TaskNode::callback_motion(const kusv_msgs::kusv_CanInfo::ConstPtr& msg){
    ...
    iMotion.speed_x = (float32_t)(msg->speed_avr_r*kph2mps); //>>> rear average speed, km/h or m/s ??
    iMotion.speed_y = 0;
    iMotion.speed_z = 0;
    ...
    //ROS_INFO("Speed : %f Yaw : %f YawRateBias : %f",iMotion.speed_x,iMotion.yaw_rate,yaw_rate_bias);
